...
{
	if ((*fuso)) {
		printf(ANSI_ERROR_PREFIX " Could not allocate FUSO (POINTER IS NOT NULL)\n"); 
		return 1;
	}

	if (!(*fuso = rtai_malloc(rt_get_name(0), sizeof(fuso_t)))) {
		printf(ANSI_ERROR_PREFIX " Could not allocate FUSO\n");
		return 2;
	}
	(*fuso)->id = id;
	(*fuso)->sensor = 0;
...
void dealloc_fuso(fuso_t** fuso)
{
	...
	{
		if ((*fuso)->sensor) {
			rt_shm_free(rt_get_name((*fuso)->sensor));
		}
		rt_shm_free(rt_get_name((*fuso)));
		(*fuso) = 0;
	}
};
...
{
	if (!fuso) {
		printf(ANSI_ERROR_PREFIX " Could not attach FUSO to BUFFE (FUSO IS NULL)\n");
		return 1;
	}

	if (!buffer) {
		printf(ANSI_ERROR_PREFIX " Could not attach BUFFE to FUSO (BUFFER IS NULL)\n");
		return 2;
	}
	fuso->buffer = buffer;
	return 0;
...
int attach_fuso_to_controller(fuso_t* fuso, controller_t* controller)
{
	...
	fuso->mbx_filtered = controller->mbx_filtered;
	return 0;
};
